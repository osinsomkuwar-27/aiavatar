...
const BACKGROUNDS = {
  ...
  educational: [
    { id: 'e1', label: 'Classroom',  image: '/backgrounds/classroom.webp',  desc: 'Warm classroom setting' },
    { id: 'e2', label: 'Library',    image: '/backgrounds/library.jpg',      desc: 'Books, wooden shelves' },
    { id: 'e3', label: 'Whiteboard', image: '/backgrounds/whiteboard.webp',  desc: 'Clean whiteboard backdrop' },
  ],
  casual: [
    { id: 'c1', label: 'Coffee Shop',  image: '/backgrounds/coffee-shop.avif', desc: 'Warm cafe atmosphere' },
    { id: 'c2', label: 'Home Studio',  image: '/backgrounds/home-studio.jpg',  desc: 'Cozy home setup' },
  ],
  abstract: [
    { id: 'a1', label: 'Black Gradient', image: '/backgrounds/black-gradient.jpg',  desc: 'Deep black gradient' },
    { id: 'a2', label: 'Abstract Blue',  image: '/backgrounds/abstract-blue.avif',  desc: 'Deep blue abstract' },
    { id: 'a3', label: 'Purple Gradient',image: '/backgrounds/purple-gradient.jpg', desc: 'Soft purple gradient' },
    { id: 'a4', label: 'Brand Green',    image: '/backgrounds/brand-green.jpg',     desc: 'VaktarAI green accent' },
  ],
}
...
// ── This component receives personPhoto from parent (teammate's upload flow)
// ── Pass it in like: <BackgroundPage personPhoto={uploadedPhotoUrl} addToast={addToast} />
export default function BackgroundPage({ addToast, personPhoto = null, onBackgroundApply }) {
  ...
  function handleApply() {
    const bgData = bgType === 'color'
      ? { type: 'color', color: solidColor, label: solidColor.toUpperCase(), opacity }
      : bgType === 'upload' && uploadedBgUrl
      ? { type: 'upload', image: uploadedBgUrl, label: uploadedFile?.name, opacity }
      : selectedBg?.image
      ? { type: 'preset', image: selectedBg.image, label: selectedBg.label, opacity }
      : null

    if (!bgData) {
      addToast?.('Please select a background first', 'error')
      return
    }

    onBackgroundApply?.(bgData)
    setApplied(true)
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 14, overflow: 'hidden' }}>
          ...
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-display, sans-serif)', fontSize: 38, color: '#ede8d8', lineHeight: 0.95 }}>
          ...
          <div style={{ flex: 1, borderRadius: 20, overflow: 'hidden', position: 'relative', border: '0.5px solid rgba(255,255,255,0.13)', minHeight: 0 }}>
            ...
            <div style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', zIndex: 5 }}>
              {personPhoto ? (
                // Real person photo from teammate's upload
                <img
                  src={personPhoto}
                  alt="avatar"
                  style={{
                        ...
                        <div style={{
                          ...
                          {!bg.image && (
                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>+ Add image</span>
                            </div>
                          )}
                          {isSelected && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                >
                  ...
                  <p style={{ fontSize: 12, color: '#ede8d8', fontWeight: 500, textAlign: 'center' }}>
                    {uploadedFile ? uploadedFile.name : 'Upload your background image'}
                  </p>
                ...
                {uploadedBgUrl && (
                  <div style={{ borderRadius: 12, overflow: 'hidden', border: '0.5px solid rgba(255,255,255,0.13)', position: 'relative' }}>
                    <img src={uploadedBgUrl} alt="bg preview" style={{ width: '100%', height: 140, objectFit: 'cover', display: 'block' }} />
...
function AvatarPlaceholder() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, paddingBottom: 16, opacity: 0.4 }}>
      ...
      <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
